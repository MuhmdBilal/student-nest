...
function Agencies() {
        ...
        },
        {
            ...
            cell: (row) => (
                <>
                {
                    activeShow == 1 ? ( 
                        <>
                    <button className='btn btn-xxs btn-warnings me-2 mt-1 mt-1' title="Delete" onClick={() => agencyDataDelete(row._id, row.active)}><BiLogOut style={{ color: "white" }} size={20}></BiLogOut></button>
                    <button className='btn btn-xxs btn-dangers mt-1' style={{ color: "white" }} title="Delete" onClick={() => userDeletePermanent(row._id)}><MdDelete size={20} /></button>
                        ...
                        <>
                          <Link to={`/sidebar/show_single_agency_data/${row._id}`} style={{ textDecoration: "none" }}><button className='btn btn-xs btn-infoss me-2 mt-1' style={{ paddibg: "0" }} title="View"><i className="fa-solid fa-eye" style={{ color: "white" }}></i></button></Link>
                    <Link to={`/sidebar/update_single_agency_data/${row._id}`} style={{ textDecoration: "none" }}><button className='btn btn-xs btn-warnings me-2 mt-1' style={{ paddibg: "0" }} title="Update"><i className="fa-solid fa-pencil" style={{ color: "white" }}></i></button></Link>
                    <button className='btn btn-xxs btn-dangers mt-1' title="Delete" onClick={() => agencyDataDelete(row._id, row.active)}><i className="fa-solid fa-xmark" style={{ color: "white" }}></i></button>
                        </>
                    )
                }
    ...
    const agencyData = async () => {
        try {
            ...
            await axios.post(`${BACKEND_URI}/agency`, {
                ...
                setStatus(false)
                setloading(false)
                toast.success("Agencies Add successfully")
    ...
    const agencyDataDelete = async (id, ststus) => {
        try {
            let active = ""
            if (ststus == "true") {
                ...
                await axios.put(`${BACKEND_URI}/ageny_data_delete/${id}`, { active }).then((resps) => {
                    ...
                    if (resps) {
                        toast.error("Agency Inactive successfully")
                ...
                await axios.put(`${BACKEND_URI}/ageny_data_delete/${id}`, { active }).then((resps) => {
                    if (resps) {
                        toast.success("Agency Active successfully")
                        setLoadingActive(false)
                        agencyDataGet()
                    }
                })
      ...
      const handleDataDelete =async()=>{
        try{
            setDeleteLoading(true)
               await axios.delete(`${BACKEND_URI}/parmanently_Delete_Agency/${scheduleTableId}`).then((res)=>{
                   setModalShow(false);
                   setDeleteLoading(false)
                   agencyDataGet();
                   toast.success("Agency Deleted Parmanently")
               })
        }catch(e){
            console.log("e", e);
            setDeleteLoading(false)
                            ...
                            <div className='row d-flex flex-column justify-content-center justify-content-between pt-3 pb-3 align-items-center' style={{ background: "#c7d7df", borderBottomLeftRadius: "10px" , borderBottomRightRadius: "10px"  }}>
                                ...
                                <div className='col-lg-6 text-md-start mt-2'>
                                    ...
                                    <div className="mb-3 d-flex align-items-center">
                                        ...
                                        <input type="text" className="form-control ms-3" id="exampleFormControlInput1" placeholder="Agency Name" value={title} onChange={(e) => setTitle(e.target.value)} required />
                                        <br />
                                    </div>
                                </div>
                                <div className='col-lg-6 text-md-start mt-2'>
                                    <div className="mb-3 d-flex align-items-center">
                                        ...
                                        <input type="text" className="form-control ms-3" id="exampleFormControlInput1" placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} required /><br />
                                    </div>
                                </div>
                                ...
                                <div className='col-md-11 mt-4 pt-3 pb-3 mb-5' style={{ borderBottom: "1px solid #838383", borderTop: "1px solid #838383" }}>
                                    <button className='btn btn-save me-2' onClick={agencyData}>{lodaing == true ? <Spinner animation="border" /> : <span>Save</span>} </button>
                        ...
                        <div>
                            ...
                            <div className='row d-flex justify-content-center justify-content-between pt-3 pb-3 align-items-center ps-md-3 pe-md-3' style={{ background: "#c7d7df"}}>
                                <div className='col-lg-5 text-md-start d-flex align-items-center'>
                                    <button className='btn btn-primaryadd me-md-3' onClick={changeAgenciews}><i className="fa-solid fa-plus" ></i> Add New Agencies</button>
                                </div>
                            ...
                            <span className='text-start  ms-md-4'>Total Users: {sckelton == true ? <Placeholder as="s" animation="glow"><Placeholder xs={1} /></Placeholder>: <span>{activeShow == 1 ? <span>{totalAgenciesInactive}</span> : <span>{totalAgenciesActive}</span>}</span>} </span>
                                    ...
                                    </div> : <>
                                        {
                                            activeShow == 1 ? (
                                                ...
                                                <DataTable
                                                    ...
                                                    striped
                                                    subHeaderComponent={
                                                        <input type="text" placeholder='Search Agency' className=' form-control' style={{ width: '28%' }} value={search} onChange={(e) => setSearch(e.target.value)} />
                                                    }
...

{
        modalShow ? (
          ...
          >
            ...
            <Modal.Body>
              Are you sure you want to delete Agency. This will be remove Permanently !
            </Modal.Body>
            <Modal.Footer>
