...
function Subject() {
        ...
        },
        {
            ...
            cell: (row) => (
                <>
                {
                    activeShow == 1 ? (
                        <>
                    <button className='btn btn-xxs btn-warnings me-2 mt-1' title="Delete" onClick={() => subjectDataDelete(row._id, row.active)}><BiLogOut style={{ color: "white" }} size={20}></BiLogOut></button>
                    <button className='btn btn-xxs btn-dangers mt-1' title="Delete" style={{ color: "white" }} onClick={() => userDeletePermanent(row._id)}><MdDelete size={20} /></button>                        
                        ...
                        <>
                        <Link to={`/sidebar/show_single_subject_data/${row._id}`} style={{ textDecoration: "none" }}><button className='btn btn-xs btn-infoss me-2 mt-1' style={{ paddibg: "0" }} title="View"><i class="fa-solid fa-eye" style={{ color: "white" }}></i></button></Link>
                    <Link to={`/sidebar/update_single_subject_data/${row._id}`} style={{ textDecoration: "none" }}><button className='btn btn-xs btn-warnings me-2 mt-1' style={{ paddibg: "0" }} title="Update"><i class="fa-solid fa-pencil" style={{ color: "white" }}></i></button></Link>
                    <button className='btn btn-xxs btn-dangers mt-1' title="Delete" onClick={() => subjectDataDelete(row._id, row.active)}><i class="fa-solid fa-xmark" style={{ color: "white" }}></i></button>
                        </>
                    )
                }
    ...
    const subjectData = async () => {
        try {
            ...
            await axios.post(`${BACKEND_URI}/subjects`, {
                active,
                title,
                description,
                currentTime
    ...
    const subjectDataGet = async () => {
        try {
            setTableLoading(true)
            setSckelton(true)
            await axios.get(`${BACKEND_URI}/subjects`).then((resp) => {
                let arry = []
                let arryfalse = []
    ...
    const subjectDataDelete = async (id, ststus) => {
        try {
            let active = "";
            if (ststus == "true") {
                setLoadingActive(true)
                active = false
                await axios.put(`${BACKEND_URI}/subjects_data_delete/${id}`, { active }).then((resps) => {
                    if (resps) {
                        toast.error("Subject Inactive successfully")
                ...
                await axios.put(`${BACKEND_URI}/subjects_data_delete/${id}`, { active }).then((resps) => {
                    if (resps) {
                        toast.success("Subject Active successfully")
                        setLoadingActive(false)
                        subjectDataGet()
                    }
                })
  ...
  const handleDataDelete =async()=>{
    try{
        setDeleteLoading(true)
           await axios.delete(`${BACKEND_URI}/parmanently_Delete_Subject/${scheduleTableId}`).then((res)=>{
               setModalShow(false);
               setDeleteLoading(false)
               subjectDataGet()
               toast.success("Subject Deleted Parmanently")
           })
    }catch(e){
        console.log("e", e);
        setDeleteLoading(false)
            ...
            </div>}
            {
                status ? (
                    <div>
                        <div className='row user-box-1'>
                            <div className='col-lg-12 col-12  d-flex justify-content-center  justify-content-between align-items-center pt-3 pb-3'>
                                <h4 className='user-h4 mt-2'>NEW SUBJECT</h4>
                            </div>
                        </div>
                        <div className='row d-flex flex-column justify-content-center justify-content-between pt-3 pb-3 align-items-center' style={{ background: "#c7d7df", borderBottomLeftRadius: "10px" , borderBottomRightRadius: "10px"  }}>
                            <div className='col-lg-6 text-md-start mt-2'>
                                ...
                                <div className="mb-3 d-flex align-items-center">
                                    ...
                                    <input type="email" className="form-control ms-3" id="exampleFormControlInput1" placeholder="Subject Name" value={title} onChange={(e) => setTitle(e.target.value)} required />
                                    <br />
                                </div>
                            </div>
                            <div className='col-lg-6 text-md-start mt-2'>
                            ...
                            <div className='col-md-11 mt-4 pt-3 pb-3 mb-5' style={{ borderBottom: "1px solid #838383", borderTop: "1px solid #838383" }}>
                                <button className='btn btn-save me-2' onClick={subjectData}>{lodaing == true ? <Spinner animation="border" /> : <span>Save</span>}</button>
                    ...
                    <div>
                        <div className='row user-box-1'>
                            <div className='col-lg-12 col-12  d-flex justify-content-center  justify-content-between align-items-center pt-3 pb-3'>
                                <h4 className='user-h4 mt-2'>SUBJECTS</h4>
                                {/* <div>
                        ...
                        <div className='row d-flex justify-content-center justify-content-between pt-3 pb-3 align-items-center ps-md-3 pe-md-3' style={{ background: "#c7d7df"  }}>
                            <div className='col-lg-5 text-md-start mt-2 d-flex align-items-center'>
                                <button className='btn btn-primaryadd me-md-3' onClick={changeAgenciews}><i class="fa-solid fa-plus"></i> Add New Subject</button>
                            </div>
                        ...
                        <span className='text-start'>Total Subjects: {sckelton == true ? <Placeholder as="s" animation="glow"><Placeholder xs={1} /></Placeholder>: <span>{activeShow == 1 ? <span>{totalAgenciesInactive}</span> : <span>{totalAgenciesActive}</span>}</span>} </span>
                                    ...
                                    {activeShow == 1 ? (
                                        ...
                                        <DataTable
                                            ...
                                            theme="solarized"
                                                    striped
                                            subHeaderComponent={
                                                <input type="text" placeholder='Search Subject' className='form-control' style={{ width: '28%' }} value={searchInactive} onChange={(e) => setSearchInactive(e.target.value)} />
                                            }
                                        ...
                                        <DataTable
                                            ...
                                            theme="solarized"
                                                    striped
                                            subHeaderComponent={
                                                <input type="text" placeholder='Search Subject' className=' form-control' style={{ width: '28%' }} value={search} onChange={(e) => setSearch(e.target.value)} />
                                            }
...

{
        modalShow ? (
          ...
          >
            ...
            <Modal.Body>
              Are you sure you want to delete Subject. This will be remove Permanently !
            </Modal.Body>
            <Modal.Footer>
